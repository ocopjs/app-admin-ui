...
class AdminUIApp {
  ...
  build({ ocop, distDir }) {
    ...
    const adminMeta = this.getAdminUIMeta(ocop);
    const compilers = [];
    const secureCompiler = webpack(
      getWebpackConfig({
        adminMeta,
        adminViews: this.getAdminViews({ ocop, includeLists: true }),
        entry: "index",
  ...
  getAdminViews({ ocop, includeLists }) {
    const { pages, hooks } = this;
    const { listViews } = includeLists
      ? ocop.getAdminViews({ schemaName: this._schemaName })
      : { listViews: {} };

    return { pages, hooks, listViews };
  }

  getMiddleware({ ocop, distDir, dev }) {
  ...
  createProdMiddleware({ distDir }) {
    ...
    if (!fs.existsSync(builtAdminRoot)) {
      throw new Error(
        "There are no Admin UI build artifacts. Please run `ocop build` before running `ocop start`",
      );
    }
  ...
  createDevMiddleware({ adminMeta, ocop }) {
