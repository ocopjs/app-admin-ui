...
const checkIsReadOnly = ({ maybeAccess, isReadOnly }) =>
  !maybeAccess.update || !!isReadOnly;
...
const ItemDetails = ({ list, item: initialData, itemErrors, onUpdate }) => {
  ...
  return (
    <Fragment>
      {itemHasChanged.current && !deleteConfirmed.current && (
        <PreventNavigation />
      )}
                ...
                <div
                  ...
                  className={
                    (field.config?.className ||
                      (isReadOnly ? "col-6" : "col-12")) + " float-left px-1"
                  }
                >
