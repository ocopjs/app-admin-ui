...
const getValues = (fieldsObject, item) =>
  mapKeys(fieldsObject, (field) => field.serialize(item));
...
const CreateItemModal = ({
  ...
  const [item, setItem] = useState(
    list.getInitialItemData({ prefill: prefillData }),
  );
  ...
  const _onCreate = useEventCallback(async (event) => {
    ...
    const hasNotChangedAndIsNotRequired = (path) => {
      const hasChanged = fieldsObject[path].hasChanged(
        initialValues,
        currentValues,
      );
    ...
    const fields = Object.values(
      omitBy(fieldsObject, (path) => !data.hasOwnProperty(path)),
    );
  ...
  const _createItemModalClose = () => {
    ...
    setItem(list.getInitialItemData({}));
    if (onClose) {
      const data = arrayToObject(creatable, "path", (field) =>
        field.serialize(item),
      );
      onClose(data);
    }
  };
          ...
          <div className="container-lg clearfix">
            <Render>
              {() => {
                ...
                captureSuspensePromises(
                  creatable.map((field) => () => field.initFieldView()),
                );
                ...
                return creatable.map((field, i) => (
                  <div
                    ...
                    className={
                      (field.config?.className || "col-12") + " float-left px-1"
                    }
                  >
...
const ConfirmModal = ({ isOpen, onConfirm, onCancel }) => {
  return (
    <Confirm isOpen={isOpen}>
      <p style={{ marginTop: 0 }}>
        Tất cả dữ liệu trong form sẽ bị hủy bỏ. Bạn chắc chứ?
      </p>
      <footer>
