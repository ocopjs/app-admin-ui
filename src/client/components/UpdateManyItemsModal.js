...
import { omit, arrayToObject, countArrays } from "@ocopjs/utils";
...
const UpdateManyModal = ({ list, items, isOpen, onUpdate, onClose }) => {
  ...
  const handleUpdate = async () => {
    ...
    const data = arrayToObject(selectedFields, "path", (field) =>
      field.serialize(item),
    );

    if (!countArrays(validationWarnings)) {
      const { errors, warnings } = await validateFields(
        selectedFields,
        item,
        data,
      );
  ...
  const options = useMemo(
    ...
    () =>
      list.fields
        .filter(
          ({ isPrimaryKey, maybeAccess }) =>
            !isPrimaryKey && !!maybeAccess.update,
        )
  ...
  return (
    ...
    >
      <FieldContainer>
        <FieldLabel
          field={{ label: "Mục cập nhật", config: { isRequired: false } }}
        />
        <FieldInput>
          <Select
            autoFocus
