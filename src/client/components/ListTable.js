...
import { captureSuspensePromises, noop } from "@ocopjs/utils";
import {
  KebabHorizontalIcon,
  LinkIcon,
  ShieldIcon,
...
const SortLink = ({
  ...
  return (
    ...
    >
      ...
      {sortable && (
        <SortDirectionArrow
          rotate={active && sortAscending ? "180deg" : "0deg"}
        />
      )}
    </HeaderCell>
  );
  ...
  const items = [
    {
      ...
      icon: <LinkIcon />,
      onClick: () =>
        copyToClipboard(`${window.location.origin}${list.fullPath}/${item.id}`),
    },
    {
      content: "Delete",
      icon: <TrashcanIcon />,
